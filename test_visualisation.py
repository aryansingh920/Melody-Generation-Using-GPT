...
Filename: test_visualisation.py
...
def create_improvement_heatmap(results: Dict[str, Dict[str, Dict[str, Any]]]):
    ...
    for i, model in enumerate(models):
        for j, test_file in enumerate(test_files):
            improvement_matrix[i,
                               j] = results[model][test_file]['improvement_ratio']
...
def create_perplexity_comparison(results: Dict[str, Dict[str, Dict[str, Any]]]):
    ...
    for i, test_file in enumerate(test_files):
        perplexities = [results[model][test_file]['perplexity']
                        for model in models]
        baseline_perplexities = [results[model][test_file]
...
def main():
    ...
    for model in results:
        ...
        for test_file, metrics in results[model].items():
            ...
            print(
                f"    Improvement over baseline: {metrics['improvement_ratio']:.2f}x")
